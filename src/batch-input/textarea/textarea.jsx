...
export default class NewTextArea extends TextArea{
    constructor(props){
        super(props);
        this.state = {
    ...
    getValueLength(value) {
        const { maxLength, cutString ,target } = this.props;
        const nv = `${value}`;
        let inputLen = this.props.getValueLength(nv);
        /**
         * 这个函数是一个空方法
         * this.props.getValueLength
            // export const noop = () => {};
            返回的strLen是一个undefined
         */
        if(target && target === 'items'){
            // 计算的是条数
            /* istanbul ignore if */
            inputLen = this.solveTextAreaValue(nv).split('\n').length;
            if (inputLen > maxLength && cutString) {
                //todo 在这里写逻辑，判断是否超出
                inputLen = inputLen.splice(0,maxLength).join('\n');
            }
        }else{
             // 计算的是字符个数
