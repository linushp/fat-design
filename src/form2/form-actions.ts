...
import {getCompositeValue, parseCompositeName, setCompositeValue} from "./helper/parseCompositeName";
...
class FormActions {
    ...
    getValue(name: string): any {
        return getCompositeValue(this.formStore, parseCompositeName(name));
    }
    ...
    setValue(name: string, value: any): any {
        setCompositeValue(this.formStore, parseCompositeName(name), value);
        return value;
    }
    ...
    getResetNames(includeNames: StringOrArray = "*", exclude: string[]): string[] {
        ...
        const filterByIncludeNames = (): string[]=>{
            if (!includeNames || includeNames ==='*' || (Array.isArray(includeNames) && includeNames.includes("*"))) {
                return allnames;
            }
            if (Array.isArray(includeNames)) {
                return allnames.filter((name) => {
                    return includeNames.includes(name);
                })
            }
        ...
        const resultByIncludeNames= filterByIncludeNames();
        if (Array.isArray(exclude) && exclude.length > 0) {
            return resultByIncludeNames.filter((t) => {
                return !exclude.includes(t);
            });
        }
        return resultByIncludeNames;
    }
    ...
    pickFormValues() {
        ...
        for (let i = 0; i < propsArray.length; i++) {
            ...
            if (item.name && item.label) {
                const parsed = parseCompositeName(item.name);
