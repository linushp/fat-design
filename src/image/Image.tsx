...
import {getOffset} from './utils/css';
...
function createDefaultPreviewMask() {
...
const ImageInternal: CompoundedComponent<ImageProps> = props => {
    const {
        ...
        prefixCls = `${prefix}image`, //'rc-image',
    ...
    const {
        ...
        mask: previewMask = createDefaultPreviewMask(),
        maskClassName,
        movable,
        icons,
        scaleStep,
    ...
    const imgCommonProps = useMemo(
        () => {
            ...
            COMMON_PROPS.forEach((prop: string) => {
                if (propsAny[prop] !== undefined) {
                    obj[prop] = propsAny[prop];
                }
            });
            return obj as ImageElementProps;
        },
