...
import {func, obj, dom, events, KEYCODE, findDOMNode, polyfill, ReactComponent} from '../../util';
import {getWidth, normalizeToArray, isSibling, isAncestor, isAvailablePos, getFirstAvaliablelChildKey} from './util';
import {isReactFragment} from "../../util/object";
...
const getNewChildren = ({children, root, mode, lastVisibleIndex, hozInLine, prefix, renderMore}) => {
    ...
    if (isReactFragment(children) && children.props) {
    ...
    const loop = (children, posPrefix, indexWrapper = {index: 0}, inlineLevel = 1) => {
        ...
        return Children.map(children, child => {
            if (isChildTypeComp(child) && 'menuChildType' in child.type) {
                let newChild;
