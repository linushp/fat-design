...
function getXY(p, staticInfo) {
  ...
  function setPointX(point, positive = true, width) {
  ...
  function setPointY(point, positive = true, height) {
    const plus = positive ? 1 : -1;
    switch (point) {
      case 't':
        basey += 0;
...
function getNewPlacements(
  ...
  const forbid = (...ps) => ps.forEach((t) => forbiddenSet.add(t));
  // 上方超出
  if (topOut) {
    forbid('t', 'tl', 'tr');
  }
  ...
  const allPlacements = Object.keys(placementMap);
  // 过滤出所有可用的
