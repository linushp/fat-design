...
class List extends Component {
    ...
    getImageListThumbnail = (imgURL, file, alt)=> {
        ...
        if (file.state === 'error') {
            img = <Icon type="cry"/>;
        } else if (file.state === 'uploading' || (file.state === 'selected' && !imgURL)) {
            img = <Icon type="picture"/>;
        } else if (typeof imgURL === "object" && imgURL && imgURL.fileSuffix) {
            img = imgURL.fileSuffix;
        } else {
            img = (
                <img
                    src={imgURL}
        ...
        // 使用自定义渲染
        if (typeof thumbnailRender === "function") {
            const img2 = thumbnailRender(img, imgURL, file);
            if (img2) {
                img = img2;
            }
        }
    ...
    getImageList(file) {
        const { extraRender, thumbnailRender, actionRender, progressProps, rtl, fileNameRender, previewOnFileName } = this.props;

        const { prefixCls, downloadURL, imgURL, size, itemCls, alt } = this.getInfo(file);

        let img = this.getImageListThumbnail(imgURL, file, alt)
    ...
    getPictureCardThumbnail = (file, isPreview, imgURL, prefixCls, alt) => {

        console.log('getPictureCardThumbnail', file.state)
        const {locale, thumbnailRender} = this.props;
        let img;
        ...
        } else if (state === 'uploading' || (state === 'selected' && !imgURL)) {
            img = (
                <div className={`${prefixCls}-list-item-handler`}>
                    <Icon type="picture"/>
    ...
    getPictureCardList(file, isPreview) {
        ...
        let img = this.getPictureCardThumbnail(file, isPreview, imgURL, prefixCls, alt);
