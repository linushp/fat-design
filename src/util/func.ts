 ...
 */
export function promiseCall(ret, success, failure: any = noop) {
    if (isPromise(ret)) {
        return ret
            .then(result => {
                success(result);
...
export function pickerDefined(obj: any): any {
    const newObj: any = {};
 ...
 * @returns {boolean}
 */
export function isStartUpper(str: string){
    if (str && str.length > 0){
        const c = str.charCodeAt(0);
        return charCodeA <= c && c <= charCodeZ;
    }
    return false;
}
...
export function toNumber(obj: any , defaultNum: number) {
...
export function wrapperFn(fn, setLoading, callback, originalInstance) {
    return (event,...args) => {

        if (!event || typeof event === 'string') {
            event = {};
            event.originalEventName = event;
        }

        event.originalInstance = originalInstance;
...
export function parseJsonObject(str: any) : any{
