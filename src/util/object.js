...
export function pickOthers(holdProps, props) {
    ...
    for (const key in props) {
        if (!_isInObj(key, holdProps, isArray)) {
            ...
            if (key === 'rtl') {
                others['dir'] = (value === true) ? 'rtl' : undefined;
            } else {
                others[key] = value;
            }
        }
 ...
 * @type {Set<string>}
 */
const pickBasicPropsSet = new Set([
    "id",
    "class",
    "style",
    "src",
...
export function isBasicProps(name){
    if (pickBasicPropsSet.has(name) || name.startsWith("on") || name.startsWith('data')){
        return true;
    }
    const lowerCase = name.toLowerCase();
    return pickBasicPropsSet.has(lowerCase);
}
export function pickBasicProps(props){
    if (!props) {
        return {};
    }
    const others = {};
...
/**
 * 对象继承。如果child属性不存在，就使用parent的属性。
 * @param childObj
 * @param parentObj
 * @param keys
 * @returns {*}
 */
export function objectExtend(childObj, parentObj, keys){
    for (let i = 0; i < keys.length; i++) {
        const key = keys[i];
        if (isNil(childObj[key])){
